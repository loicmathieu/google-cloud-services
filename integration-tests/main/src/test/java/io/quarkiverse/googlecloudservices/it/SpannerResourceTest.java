...
public class SpannerResourceTest {
    ...
    @Test
    @SetEnvironmentVariable(key = "SPANNER_EMULATOR_HOST", value = "localhost:9010")
    public void testSpanner() throws ExecutionException, InterruptedException {
        // create the table DDL
        spanner.getDatabaseAdminClient()
                .updateDatabaseDdl("test-instance", "test-database",
                    Arrays.asList("CREATE TABLE Singers ( SingerId INT64 NOT NULL, FirstName STRING(1024), LastName STRING(1024), SingerInfo BYTES(MAX) ) PRIMARY KEY (SingerId)"), null)
                .get();
